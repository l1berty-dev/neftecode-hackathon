...
## Актуальный двухрежимный dashboard

Стартовая вкладка — «Модельные сценарии»: редактируемые входы АВТ, P8/T11/F19,
спецификация, резервуары и 2-EHN; результат показан цепочкой причина → действие →
качество → рецепт → ограничения. Допущения/trace свернуты. История modelled runs
загружается из PostgreSQL. «Исторический replay» сохранён второй вкладкой без изменения
смысла. `npm run e2e` проверяет пять preset-сценариев в headless Chrome и сохраняет
реальные screenshots в `docs/assets/`.
