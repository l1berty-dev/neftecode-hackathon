...
| --- | --- | --- |
| Replay repository и transactional advance | разработчик 2, F | Не реализовано; схема-заготовка есть |
| Полная persistence-транзакция решения и evaluations | разработчик 2, F | Основа репозиториев есть; общий API-flow ещё не подключён |
...
Следующий корректный порядок: developer 2 F persistence/replay параллельно с developer 1 F
CLI/FastAPI, затем интеграционный PostgreSQL E2E, OpenAPI → frontend G и финальные проверки.

## Проверки аудита

- `uv run ruff check .`
- `uv run ruff format --check .`
- `uv run pytest`: 174 passed, 2 skipped; пропущены только opt-in PostgreSQL tests.
