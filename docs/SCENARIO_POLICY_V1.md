...
Исторически подготовительный Coordinator отказывался ранжировать admissible
evaluations. После этапа D он генерирует кандидаты, оценивает через единый evaluator
и возвращает один из четырёх исходов; подробности:
[CANDIDATE_SELECTION_V1.md](CANDIDATE_SELECTION_V1.md). Этап E полного объяснения
и trace реализован: [COORDINATOR_V1.md](COORDINATOR_V1.md).
Продолжает работать отдельный ScenarioEvaluator.evaluate.
...
не используются для подбора границ. Continuation model и required-input manifest уже переданы.
Train-only диапазоны, численные единицы/шкалы и action support управлений, утверждённая interval
policy и inventory обязательных hard checks ещё нужны от совместной работы.
Исторически код первого разработчика не менялся в этапе B. В последующей передаче
он реализовал задержку ЛИМС 240 минут и SnapshotProvider, затем baseline-модель
и manifest. Численные единицы/шкалы действий ещё не переданы. Этап C разработчика 2 реализован:
[SEVERITY_EFFICIENCY_V1.md](SEVERITY_EFFICIENCY_V1.md). Статический индекс доступен
только с подтверждённой нормировкой; текущий расход продукта отделён от будущего
выпуска. Будущие выпуск/затраты остаются неизвестными, переход не оценён.
